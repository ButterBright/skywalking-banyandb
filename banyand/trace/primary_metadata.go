...
func (pbm *primaryBlockMetadata) mustWriteBlock(data []byte, traceID string, minTimestamp, maxTimestamp int64, sw *writers) {
	pbm.traceIDLen = uint32(len(traceID))
	pbm.traceID = traceID
...
func (pbm *primaryBlockMetadata) unmarshal(src []byte) ([]byte, error) {
	...
	pbm.traceID = string(src[:pbm.traceIDLen])
